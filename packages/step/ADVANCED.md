# @secorto/step — Advanced Usage

Complete API reference, integration patterns, and design details for library authors and advanced integrations.
...
This approach keeps the integration explicit and the library framework‑agnostic.
...
## Contract Steps as API Definition Framework

Contract steps model API contracts as observable, testable units. Each contract:

- Defines an **origin** (data source, API endpoint, service call)
- Defines a **transform** (schema validation, normalization, parsing)
...
- Documenting data flow visibly in test output
...
export const userAPI = {
  ...
  create: (data: UserInput) =>
    contractStep(
      'post user to /api/users',
      async () => fetch('/api/users', { method: 'POST', body: JSON.stringify(data) }).then(r => r.json()),
      (json) => UserSchema.parse(json)
    ),
}
```

Tests can use, inspect, or decompose these contracts:

```ts
// Standard flow
const user = await userAPI.fetch('123')

// Inspect raw before parsing
const raw = await userAPI.fetch('123').raw()
if (!raw.ok) { /* handle */ }

// Compare raw vs parsed
const { raw, parsed } = await userAPI.fetch('123').detailed()
```
