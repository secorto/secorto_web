...
test.describe('A11y - Blog', { tag: ['@a11y', '@blog'] }, () => {
  languageKeys.forEach((locale) => {
    test(`blog list a11y (${locale})`, { tag: [`@${locale}`] }, async ({ Given, When, Then, page }) => {
    ...
    test(`blog tag a11y (${locale})`, { tag: [`@${locale}`] }, async ({ Given, When, Then, page }) => {
    ...
    test(`blog detail a11y (${locale})`, { tag: [`@${locale}`] }, async ({ Given, When, Then, page }) => {
