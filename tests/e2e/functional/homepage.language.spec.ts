...
import { userInHome } from '@tests/pages/HomeLanguageUserJourney'
...
test.describe('Homepage language switch', { tag: ['@functional', '@i18n'] }, () => {
  for (const f of fixtures) {
    test(`switch from ${f.from} to ${f.to} via UI`, async ({ Given, When, Then, page }) => {
      const journey = await Given(userInHome(page, f.from))
      await Then(journey.shouldHaveLanguageOption(f.to))
